...
public class WbXmlParser {
    ...
    /**
     * Some languages do weird things like encode another document into an
     * Opaque data (SyncML for instance). This property let the parsing
     * manage more than one language specification.
     */
    private List<WbXmlDefinition> defsUsed;
    ...
    public WbXmlParser(InputStream is) {
        ...
        currentByte = null;
        nextByte = null;
        this.defsUsed = new ArrayList<WbXmlDefinition>();
    }
     ...
     * 
     * @return The publicId or the index in the table in case string 
     * representation. In case of string representation StrTbl has not been read
     * yet, so no language definition is still associated to the parser.
     ...
     * @return The WbXML content (it should be a string content)
     * @throws IOException Some error reading the stream or locating/executing the plugin
     */
    public WbXmlContent parseOpaqueAttr(String attrName) throws IOException {
        // first get the plugin for the attr
        OpaquePlugin plugin = doc.getDefinition().locateAttrPlugin(attrName);
        if (plugin == null) {
            // read as a string opaque
    ...
    /**
     * Once a plugin is located this method reads the byte array that represents
     * the opaque data and gives it to the plugin parse method. It is called 
     * by the two previous methods (parseOpaqueTag and parseOpaqueAttr).
     * @param plugin The plugin to execute
     * @return The WbXMlContent obtained from the plugin
     * @throws IOException Some error reading the stream or executing the plugin
     */
    public WbXmlContent parseOpaque(OpaquePlugin plugin) throws IOException {
        // read the OPAQUE tag
        read();
    ...
    public List<String> parseAttributeValues(String attrName) throws IOException {
        ...
        while (cont) {
            ...
            if (WbXmlLiterals.STR_I == currentByte) {
                ...
                readBackwards();
                WbXmlContent c = parseOpaqueAttr(attrName);
                if (c != null && c.isString() && c.getString() != null && !c.getString().isEmpty()) {
    ...
    public WbXmlDocument parse() throws IOException {
        ...
        defsUsed.add(doc.getDefinition());
        // read the body
    ...
    /**
     * As it is said in the class some languages do weird things like encode
     * another WBXML document in an opaque data. For that the parse object
     * can store more than one definition (the standard defines only one
     * definition per document but...)
     * @return The definitions used in the parsing (usually one, exactly the
     * one of the document)
     */
    public List<WbXmlDefinition> getdefinitionsUsed() {
        return this.defsUsed;
    }
     ...
     * @see WbXmlParser#getdefinitionsUsed()
     */
     ...
     * used in the parsing. The method uses all the language definitions
     * used in the parsing instead of the document one.
     * @param namespaceURI The namespaceURI to locate a prefix from
     * @return The prefix or null
     */
    public String getPrefix(String namespaceURI) {
        for (WbXmlDefinition def: defsUsed) {
    ...
    public String getNamespaceURI(String prefix) {
        for (WbXmlDefinition def: defsUsed) {
            String namespaceURI = def.getNamespaceURI(prefix);
            if (prefix != null) {
                return namespaceURI;
            }
        }
    ...
    /**
     * Utility method to obtain an attribute definition from all the possible definitions
     * used in the parsing. The method uses all the language definitions
     * used in the parsing instead of the document one.
     * @param name The attribute name (prefixed!) to locate
     * @return The attribute definition or null if not found
     */
    public Set<WbXmlAttributeDef> locateAttribute(String name) {
        for (WbXmlDefinition def: defsUsed) {
            Set<WbXmlAttributeDef> attrDefs = def.locateAttribute(name);
            if (attrDefs != null) {
                return attrDefs;
            }
        }
    ...
    /**
     * Utility method to obtain a TAG definition from all the possible definitions
     * used in the parsing. The method uses all the language definitions
     * used in the parsing instead of the document one.
     * @param tag The tag to locate (prefixed!)
     * @return The tag definition or null if not found
     */
    public WbXmlTagDef locateTag(String tag) {
        for (WbXmlDefinition def: defsUsed) {
            WbXmlTagDef tagDef = def.locateTag(tag);
            if (tagDef != null) {
                return tagDef;
            }
        }
    ...
    /**
     * Method that searches over all the language definitions used in the parsing
     * the namespaces for a possible root tag.
     * @param tag The root tag to locate namespaces 
     * @return All the namespaces that the language defines if the tag is the root
     * tag of that definition
     */
    public Collection<WbXmlNamespaceDef> getNamespacesForTag(String tag) {
        for (WbXmlDefinition def: defsUsed) {
            String root = def.getRootName();
            if (tag.equals(root)) {
