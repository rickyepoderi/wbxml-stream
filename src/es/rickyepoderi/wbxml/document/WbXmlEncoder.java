...
public class WbXmlEncoder {
    ...
    public void encode(WbXmlDocument doc) throws IOException {
        ...
        if (doc.getDefinition().getPublicId() == 0L) {
            // unknown wbxml public id => write the xml public id using strtbl
            writeReferenceString(doc.getDefinition().getXmlPublicId());
        }
    ...
    public void encode(WbXmlElement element) throws IOException {
        ...
        if (!element.isContentsEmpty()) {
            ...
            for (WbXmlContent content : element.getContents()) {
                OpaquePlugin plugin = getDefinition().locateTagPlugin(element.getTag());
                if (plugin != null) {
                    plugin.encode(this, content);
                } else {
                    encode(content);
                }
            }
     ...
     * @throws IOException  Some error writing to the stream
     */
    public void encode(WbXmlAttribute attr) throws IOException {
        ...
        for (String v: attr.getValues()) {
            ...
            OpaquePlugin plugin = null;
            if (def != null ) {
                plugin = getDefinition().locateAttrPlugin(def.getNameWithPrefix());
            }
            if (plugin != null) {
                plugin.encode(this, new WbXmlContent(v));
            } else {
                WbXmlAttributeValueDef valueAttrDef = getDefinition().locateAttributeValue(v);
                if (valueAttrDef != null) {
                    ...
                    WbXmlExtensionDef extDef = getDefinition().locateExtension(v);
                    if (extDef != null) {
                        write(WbXmlLiterals.EXT_T_0);
                        writeUnsignedInteger(extDef.getToken());
                    } else {
