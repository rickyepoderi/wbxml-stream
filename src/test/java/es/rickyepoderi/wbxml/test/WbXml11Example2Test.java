...
/**
 * This test tries to parse/encode the example 2 in the documentation of
 * WBXML 1.0. There is no example for 1.2 or 1.3 because just changes the version.
 * 
 * @author ricky
 */
public class WbXml11Example2Test extends GenericDirectoryTester {
    
    public WbXml11Example2Test() throws IOException {
        super("src/test/examples/spec-examples/example2-11", null);
        if (WbXmlInitialization.getDefinitionByName("WBXML-EXAMPLE-2") == null) {
            Properties props = new Properties();
            props.load(new FileInputStream("src/test/examples/spec-examples/example2-10/wbxml.example2.properties"));
            WbXmlInitialization.addDefinition(props);
        }
        this.setDefinition(WbXmlInitialization.getDefinitionByName("WBXML-EXAMPLE-2"));
    }
